...
def run_training():
    ...
    processed_inputs, processed_target, inputs_pipeline = preprocess_data(
        train_data, data_schema
    )
    ...
    # save model
    classifier.save_model(model=model, model_path=model_path)
...
def preprocess_data(train_data, data_schema):
    ...
    print(
        "Processed train X/y data shape", processed_inputs.shape, processed_target.shape
    )
...
def train_model(train_X, train_y):
    ...
    data_based_params = classifier.get_data_based_model_params(train_X)
    # Create and train model
    model = classifier.Classifier(**data_based_params)
    # model.summary()
