...
def get_cat_and_num_vars_lists(data_schema):
    cat_vars, num_vars = [], []
    attributes = data_schema["inputDatasets"]["binaryClassificationBaseMainInput"][
        "predictorFields"
    ]
...
def get_cat_vars_with_missing_impute_for_na(data, pp_params, model_cfg):
    ...
    with_string_missing = [
        var for var in pp_params["cat_na"] if data[var].isnull().mean() >= threshold
    ]
...
def get_preprocess_params(data, data_schema, model_cfg):
    ...
    pp_params["id_field"] = data_schema["inputDatasets"][
        "binaryClassificationBaseMainInput"
    ]["idField"]

    # set the target attribute
    pp_params["target_attr_name"] = data_schema["inputDatasets"][
        "binaryClassificationBaseMainInput"
    ]["targetField"]
    ...
    pp_params["retained_vars"] = (
        [pp_params["id_field"]] + [pp_params["target_attr_name"]] + cat_vars + num_vars
    )
    ...
    with_string_missing = get_cat_vars_with_missing_impute_for_na(
        data, pp_params, model_cfg
    )
    with_freq_cat = get_cat_vars_with_frequent_cat_impute_for_na(
        data, pp_params, model_cfg
    )
    (
        pp_params["cat_na_impute_with_str_missing"],
        pp_params["cat_na_impute_with_freq"],
    ) = (with_string_missing, with_freq_cat)
